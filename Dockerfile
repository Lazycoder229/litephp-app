...
# ── Patch vendor helpers.php ──────────────────────────────────────────────────
# Fix 1: default VITE_DEV to false so prod never falls into dev mode
RUN sed -i "s/env('VITE_DEV', 'true')/env('VITE_DEV', 'false')/" \
        /var/www/html/vendor/litephp/core/Core/helpers.php

# Fix 2: hardcode manifest paths so public_path() resolution doesn't matter
RUN sed -i \
...
RUN echo "=== Verifying patches ===" && \
    grep "VITE_DEV" /var/www/html/vendor/litephp/core/Core/helpers.php && \
    grep "manifest.json" /var/www/html/vendor/litephp/core/Core/helpers.php
# ─────────────────────────────────────────────────────────────────────────────
